...
class Regression:
    ...
    def gpsPreprocess(self, df_gps):
        ...
        df_gps["Row"] = df_gps["Row"].astype("float")
    ...
    def timelog2Gps(self, df_timelog):
        ...
        df_timelog2gps["Row"] = 0   #timelog has no row output
...
#        used to be the last column
        ...
        df_locate_res["Row"] = df_locate_res["Row"].astype("float")
...
#        plt.xlabel("Frame")
    ...
    def plotRegressResult(self, df_gps, df_locate_res):
        ...
        df_regression.columns = ["Image_id", "State", "X_loc", "Y_loc","Z_loc",
                                 "Yaw_loc", "Row_loc", "Pitch_loc", "Matches",
                                 "X_gps", "Y_gps", "Z_gps", "Row_gps", "Pitch_gps",
                                 "Yaw_gps", "Conf_Score"]
...
#        df_regression["Z_diff"][df_regression["State"] == 0] = -0.1       
    ...
    def plotRegressResultInVehicleCoord(self, df_gps, df_locate_res):
...
#        df_locate_res["vague_id"] = df_locate_res["Image_id"].str[:14]
        ...
        df_regression.columns = ["Image_id", "State", "X_loc", "Y_loc","Z_loc",
                                 "Yaw_loc", "Row_loc", "Pitch_loc", "Matches", "Vague_id","Image_gps_id",
                                 "X_gps", "Y_gps", "Z_gps", "Row_gps", "Pitch_gps",
                                 "Yaw_gps", "Conf_Score"]
        ...
        df_regression["Yaw_diff_arc"] = abs(df_regression["Yaw_loc"] - df_regression["Yaw_gps"])
        # difference in yaw should be less or equal than 180 degree
        df_regression.loc[df_regression.Yaw_diff_arc >= np.pi, "Yaw_diff_arc"] -= np.pi
        # arc to angle 
        ...
        df_regression["Vert_diff"] = np.sin(df_regression["Yaw_diff_arc"]) * df_regression["X_diff"] + \
                                     (-1) * np.cos(df_regression["Yaw_diff_arc"]) * df_regression["Y_diff"]
        df_regression["Hori_diff"] = np.cos(df_regression["Yaw_diff_arc"]) * df_regression["X_diff"] + \
                                     (+1) * np.sin(df_regression["Yaw_diff_arc"]) * df_regression["Y_diff"]                                        
        
        # all different needs to be set in absolute distance
        df_regression["Vert_diff"] = abs(df_regression["Vert_diff"])
        df_regression["Hori_diff"] = abs(df_regression["Hori_diff"])
        ...
        threshold = 2
        thr = [threshold] * df_regression.shape[0]
        plt.plot(thr, "r")
        plotName = ["Yaw_diff", "thr = " + str(threshold)]
        plt.legend(tuple(plotName), loc='upper right')
        plt.title("Abs difference in Yaw (degree) " + ' mean: %.2f'%df_regression["Yaw_diff_ang"][df_regression["Yaw_diff_ang"]>-0.1].mean() \
                    + " Var: %.2f"%df_regression["Yaw_diff_ang"][df_regression["Yaw_diff_ang"]>-0.1].var())
        
        
        plt.subplot(3, 1, 2)
        Vert_diff = np.array(df_regression["Vert_diff"])
        plt.plot(Vert_diff)
        plt.ylabel("Vert_diff(m)")
        threshold = 0.5
        thr = [threshold] * df_regression.shape[0]
        plt.plot(thr, "r")
        plotName = ["Vert_diff", "thr = " + str(threshold)]
        plt.legend(tuple(plotName), loc='upper right')
        plt.title("Abs difference in Vert_diff (m) " + ' mean: %.2f'%df_regression["Vert_diff"][df_regression["Vert_diff"]>-0.1].mean() \
                    + " Var: %.2f"%df_regression["Vert_diff"][df_regression["Vert_diff"]>-0.1].var())        
        
        plt.subplot(3, 1, 3)
        Hori_diff = np.array(df_regression["Hori_diff"])
        plt.plot(Hori_diff)
        plt.ylabel("Hori_diff(m)")
        threshold = 0.5
        thr = [threshold] * df_regression.shape[0]
        plt.plot(thr, "r")
        plotName = ["Hori_diff", "thr = " + str(threshold)]
        plt.legend(tuple(plotName), loc='upper right')
        plt.title("Abs difference in Hori_diff (m) " + ' mean: %.2f'%df_regression["Hori_diff"][df_regression["Hori_diff"]>-0.1].mean() \
                    + " Var: %.2f"%df_regression["Hori_diff"][df_regression["Hori_diff"]>-0.1].var())        
