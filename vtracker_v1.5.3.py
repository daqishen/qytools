...
class Help(object):
    ...
    def using_tutorial():
        ...
        TEST_UNIT_4:
            ...
            TIMELOG_DIR : timelog.txt
            GPS_DIR :
            REGRESSION_SHIFT : [-0.435, -0.08, 1.8, 0, 0, 88.92]
        ...
        Please use Mode 4
        To check what Mode 3 can do, check version 1.4
        
        ------------------------------------------------------------------
        TEST_MODE : 4 
        To activate this mode, set TEST_MODE :4 and ENABLE : 1

        Mode 4 tells the information in vehicle coordinate, including the Yaw, vertical and horizontal.
        This mode can support both gps file and timelog file, but only process gps file if user add both of them.
        The regression shift can be used for parameter correction.
        Notice:
        ...
        ------------------------------------------------------------------
        TEST_MODE : 5 
        To activate this mode, set TEST_MODE :5 and ENABLE : 1
        
        Mode 5 can plot the tracking result in a 2D map, different color represent different matching number
        This mode is design for making decision in vslam mapping refine and camera selection
       
        @author: qy
        """
        return
    
    def how_to_use_this_file(self):
...
#        plt.show()
    ...
    def common_log_analysis(self):
        
        '''
        Analysing log from common.h.
        '''
...
class Regression(object):
    ...
    def gps_preprocess(self, df_gps):
        
        '''
        Preprocess gps log.
        Args:
            df_gps: dataframe from gps log.
        
        Returns:
            df_gps: modified gps.
            
        The function will create a csv file "df_gps.csv" .
        User can check the preprocess result from this csv file.
        '''
    ...
    def gps_shift(self, df_gps, regression_shift):
        
        '''
        Switch to the vehicle-centered coordinate.
        For UOS coordinate, the heading base is 90 degree, 
        when the vehicle is heading to east, the Yaw is 0 degree.
        This function will only modify the angle shift
        
        Args:
            df_gps: gps dataframe.
            regression_shift: correction parameters, including X, Y, Z,
                              Roll, Pitch, Yall.
        Returns:
            df_gps: gps dataframe, with modified Roll, Pitch, Yaw.
    ...
    def timelog_2_gps(self, df_timelog):
        
        '''
        Adapt timelog form to gps form, timelog is mainly using for mono slam, 
        gps is mainly for fish eyes.
        
        Args:
            df_timelog: dataframe.
            
        Return:
            df_timelog_2_gps: dataframe, suit for gps data form.
        ...
        if df_timelog.shape[1] == 7:
            ...
            df_timelog.columns = ["IMG_DIR", "Image_id", "Yaw", 
                                  "X", "Y", "Conf_Score", "Time"]
            df_timelog["Z"] = ''
        else:
      
            df_timelog.columns = ["IMG_DIR", "Image_id", "Yaw", 
                                  "X", "Y", "Conf_Score", "Time", "Z"]
        df_timelog.dropna()
        ...
        # rearrange datafame
        df_timelog_2_gps = pd.DataFrame(df_timelog["Image_id"])
        df_timelog_2_gps['X'] = df_timelog["X"]
        df_timelog_2_gps["Y"] = df_timelog["Y"]
        df_timelog_2_gps["Z"] = df_timelog["Z"]
        df_timelog_2_gps["Yaw"] = df_timelog["Yaw"]
        df_timelog_2_gps["Roll"] = 0   #timelog has no Roll output
        df_timelog_2_gps["Pitch"] = 0  #timelog has no pitch output       
        df_timelog_2_gps["Conf_Score"] = df_timelog["Conf_Score"] == 4  # gps confident score = 4 means reliable
        df_timelog_2_gps["Conf_Score"] = df_timelog_2_gps["Conf_Score"].astype("float")
        
#        df_timelog_2_gps.to_csv("df_timelog_2_gps.csv")
        
        return df_timelog_2_gps
    
    def vtracker_preprocess(self, df_vtracker):
        
        '''
        Process vtracker log, the result shows the information from vslam.
        The output include image id, state
        
        Args:
            df_vtracker: dataframe vtracker
            
        Return:
            df_locate_res: vtracker information, including image id, state, x,
                           y, z, roll, pitch, yaw, matches
...
#        used to be the last column
    ...
    def plot_track_result(self, df_locate_res):
        
        '''
        Plot tracker result: tracking state, matching number
        
        Args:
            df_locate_res: dataframe, location result.
        '''
...
#        plt.xlabel("Frame")
    ...
    def plot_track_result_2D(self, df_locate_res, name="VSLAM Matching Report"):
        
        '''
        Plot track result in 2D map, different colors represent different 
        matches number.
        
        Args:
            df_locate_res: dataframe, vslam location result
            
        '''
        
        df_valid_loc = df_locate_res[df_locate_res["State"] == 1].copy()
        tracking_rate = df_valid_loc.shape[0]/df_locate_res.shape[0]
        
        X = np.array(df_valid_loc["X"])
        Y = np.array(df_valid_loc["Y"])
        Matches = np.array(df_valid_loc["Matches"])
        if len(Matches) == 0:
            print("no valid VSLAM location!")
            return
        colors = ['blue' for i in range(len(Matches))]
        
        # if matches < 50, there will be no location info
        M_50_100_x = []
        M_50_100_y = []
        M_50_100_matches = []
        M_100_200_x = []
        ...
        for i in range(len(colors)):
            if Matches[i] < 100:
                M_50_100_x.append(X[i])
                M_50_100_y.append(Y[i])
                M_50_100_matches.append(Matches[i])
            elif Matches[i] < 200:
                M_100_200_x.append(X[i])
                M_100_200_y.append(Y[i])
                M_100_200_matches.append(Matches[i])
            elif Matches[i] < 300:
                M_200_300_x.append(X[i])
                M_200_300_y.append(Y[i])
                M_200_300_matches.append(Matches[i])
            elif Matches[i] < 400:
                M_300_400_x.append(X[i])
                M_300_400_y.append(Y[i])
                M_300_400_matches.append(Matches[i])
            else:
                M_400_plus_x.append(X[i])
                M_400_plus_y.append(Y[i])
                M_400_plus_matches.append(Matches[i])
    
        M_50_100_matches = np.array(M_50_100_matches)  
        M_100_200_matches = np.array(M_100_200_matches)
        M_200_300_matches = np.array(M_200_300_matches)
        M_300_400_matches = np.array(M_300_400_matches)
        M_400_plus_matches = np.array(M_400_plus_matches)

        plt.scatter(M_50_100_x,M_50_100_y,c='r',marker='.')
        plt.scatter(M_100_200_x,M_100_200_y,c='#CCFFFF',marker='.')
        plt.scatter(M_200_300_x,M_200_300_y,c='#33CCFF',marker='.')
        plt.scatter(M_300_400_x,M_300_400_y,c='#3399FF',marker='.')
        plt.scatter(M_400_plus_x,M_400_plus_y,c='b',marker='.')
        
        plt.title(name + "\n Tracking Rate: " + "%.3f"%(tracking_rate*100) + "%")
        plt.legend(("Matches: 50-100","Matches: 100-200","Matches: 200-300","Matches: 300-400","Matches: 400+"))
        plt.xlabel("X")
        plt.ylabel("Y")
        plt.show()
    ...
    def plot_regress_result(self, df_gps, df_locate_res):
        
        """
        Plot regression result in world coordinate
        
        Args:
            df_gps: dataframe gps location, which is supposed to be groundtruth
            df_locate_res: dataframe vslam locate result
        """
        ...
        df_regression.columns = ["Image_id", "State", "X_loc", "Y_loc","Z_loc",
                                 ...
                                 "X_gps", "Y_gps", "Z_gps", "Yaw_gps", "Roll_gps",
                                 "Pitch_gps", "Conf_Score"]
...
#        df_regression["Z_diff"][df_regression["State"] == 0] = -0.1       
    ...
    def plot_regress_result_in_vehicle_coord(self, df_gps, df_locate_res, x_shift = 0, y_shift = 0):
        
        '''
        Plot regression result in vehicle coordinate with regression correction
        
        Args:
            df_gps: dataframe gps location, supposed to be ground truth
            df_locate_res: vslam location result
            x_shift: verticle shift in vehicle coordinate
            y_shift: horizontal shift in vehicle coordinate
        '''
...
#        df_locate_res["vague_id"] = df_locate_res["Image_id"].str[:14]
        ...
        df_regression = df_regression[df_regression["Conf_Score"] == 1].copy()
...
#        df_regression["Vehicle_y"] = abs(df_regression["Vehicle_y"]) 
        ...
        valid_frame_num = df_regression.shape[0]
        # cdf :cumulative distribution function
        vehicle_x_error = [0,0.2,0.5,1]
        vehicle_y_error = [0,0.4,1,2]
        vehicle_x_cdf = [0]
        vehicle_y_cdf = [0]
        vehicle_x_dist = []
        vehicle_y_dist = []
        for i in range(len(vehicle_x_error)-1):
            vehicle_x_cdf.append(df_regression[abs(df_regression["Vehicle_x"])<=vehicle_x_error[i+1]].shape[0])
        vehicle_x_cdf.append(valid_frame_num)
        for i in range(len(vehicle_x_cdf)-1):
            vehicle_x_dist.append(vehicle_x_cdf[i+1] - vehicle_x_cdf[i])
        for i in range(len(vehicle_y_error)-1):
            vehicle_y_cdf.append(df_regression[abs(df_regression["Vehicle_y"])<=vehicle_y_error[i+1]].shape[0])
        vehicle_y_cdf.append(valid_frame_num)
        for i in range(len(vehicle_y_cdf)-1):
            vehicle_y_dist.append(vehicle_y_cdf[i+1] - vehicle_y_cdf[i])
            
#        print(vehicle_x_cdf, vehicle_x_dist)
#        print(vehicle_y_cdf, vehicle_y_dist)

        plt.subplot(2,1,1)
        labels_x = []
        for i in range(len(vehicle_x_error)-1):
            labels_x.append(str(vehicle_x_error[i]) + '~' + str(vehicle_x_error[i+1]) + 'm')
        labels_x.append('>=' + str(vehicle_x_error[-1]) + 'm')
        plt.title("Error distribution in vehicle x direction")
        plt.pie(vehicle_x_dist,startangle=90,autopct='%1.0f%%')
        plt.legend(tuple(labels_x),loc="right",bbox_to_anchor=(2,0.5))
        
        plt.subplot(2,1,2)
        labels_y = []
        for i in range(len(vehicle_y_error)-1):
            labels_y.append(str(vehicle_y_error[i]) + '~' + str(vehicle_y_error[i+1]) + 'm')
        labels_y.append('>=' + str(vehicle_y_error[-1]) + 'm')
        plt.title("Error distribution in vehicle y direction")
        plt.pie(vehicle_y_dist,startangle=90,autopct='%1.0f%%')
        plt.legend(tuple(labels_y),loc="right",bbox_to_anchor=(2,0.5))
        plt.show()
        return 
...
def Process(config):
    ...
    for ele in config:
        ...
        if test_unit["ENABLE"] == True:
            ...
            mode_process(test_unit)
    return

def mode_process(test_unit):
    
    '''
    Process test_unit
    
    Args:
        test_unit: shoule be in yaml form
    '''
    ...
    if test_unit["TEST_MODE"] == 0:
        ...
        try:
            ...
            Help().how_to_use_this_file()
            return
        
        try:
            relocateCheck = Relocate(df_raw)
            relocateCheck.tracker_analysis()
            plt.show()
            relocateCheck.common_log_analysis()
            plt.show()
        except:
        ...
        try:
            ...
            df_locate_res = regressionCheck.vtracker_preprocess(df_vtracker)
        except:
        ...
        try:
            ...
            df_gps = regressionCheck.gps_preprocess(df_gps)
        except:
        ...
        if Fvtracker:
            regressionCheck.plot_track_result(df_locate_res)
        if Fgps:
            regressionCheck.plot_regress_result(df_gps, df_locate_res)
        ...
        try:
            ...
            df_gps = regressionCheck.timelog_2_gps(df_timelog)
        except:
        ...
        if Fvtracker:
            regressionCheck.plot_track_result(df_locate_res)
        if Fgps:
            regressionCheck.plot_regress_result_in_vehicle_coord(df_gps, df_locate_res)
        else:
        ...
        if Fgps:
            ...
            df_gps = regressionCheck.gps_shift(df_gps, regression_shift)
            regressionCheck.plot_regress_result_in_vehicle_coord(df_gps, df_locate_res,regression_shift[0], regression_shift[1])
        else:
            print("no gps or timelog file detected")
            
            
    elif test_unit["TEST_MODE"] == 5:
        vtracker_logs = test_unit["VTRACKER_LOG_DIR"]
        regressionCheck = Regression()
        print(vtracker_logs)
        for vtracker_log in vtracker_logs:
            try:
                print("load vtracker log")
                df_vtracker = pd.read_csv(vtracker_log, "\t", header = None, error_bad_lines=False)      
                df_locate_res = regressionCheck.vtracker_preprocess(df_vtracker)
                regressionCheck.plot_track_result_2D(df_locate_res)
            except:
                print("vtracker log preprocess failed, please check log dir")
